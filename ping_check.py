...
class ping():
    ...
    def dnac_telnet_set1(self, con):
        cur = con.cursor()
        print("Telnet check on DNAC-SET-1 topology devices")
        cur.execute('select ipaddress, username, password, enablepassword, devicetype from DEVICES where topology="DNAC-TOP-1"')
        ...
        for value in devicelist:
            ...
            try:
                device = ConnectHandler(device_type=devicetype, ip=value[0], username=value[1], password=value[2], secret=value[3])
                device.enable()
                cur.execute('update DEVICES set telnetstatus="Reachable", telnetcolour="lightblue" where ipaddress=? and topology="DNAC-TOP-1"', [value[0]])
                con.commit()
                print("Device Connected")
                device.disconnect()
            except Exception as e:
                print(e)
                cur.execute('update DEVICES set telnetstatus="Unreachable", telnetcolour="FF0000" where ipaddress=? and topology="DNAC-TOP-1"', [value[0]])
                con.commit()

    def dnac_telnet_set2(self, con):
        cur = con.cursor()
        print("Telnet check on DNAC-SET-2 topology devices")
        ...
        for value in devicelist:
            ...
            try:
                ...
                device.enable()
                cur.execute('update DEVICES set telnetstatus="Reachable", telnetcolour="orange" where ipaddress=? and topology="DNAC-TOP-2"', [value[0]])
                con.commit()
                print("Device Connected")
                device.disconnect()
            except Exception as e:
                print(e)
                cur.execute('update DEVICES set telnetstatus="Unreachable", telnetcolour="FF0000" where ipaddress=? and topology="DNAC-TOP-2"', [value[0]])
                con.commit()
    ...
    def ping_lab(self, con):
        ...
        for ip in iplist:
            ...
            if response == 0:
                ...
                cur.execute('update DEVICES set reachability="Unreachable", pingcolour="FF0000" where ipaddress=?', [ip])
                con.commit()
    def ping_dnacset1(self, con):
        cur = con.cursor()
        print("Ping reachability check on DNAC-SET-1 topology devices")
        cur.execute('select ipaddress, snmpread, username, password, enablepassword from DEVICES where topology="DNAC-TOP-1"')
        ...
        for ip in iplist:
            ...
            if response == 0:
                ...
                cur.execute('update DEVICES set reachability="Reachable", pingcolour="lightblue" where ipaddress=? and topology="DNAC-TOP-1"', [ip[0]])
                con.commit()
                try:
                    device = ConnectHandler(device_type='cisco_ios_telnet', ip=ip[0], username=ip[2],
                                            password=ip[3], secret=ip[4])
                    device.enable()
                    cur.execute(
                        'update DEVICES set telnetstatus="Reachable", telnetcolour="lightblue" where ipaddress=? and topology="DNAC-TOP-1"',
                        [ip[0]])
                    con.commit()
                    print("Device Connected")
                    device.disconnect()
                    ...
                    print(e)
                    cur.execute(
                        'update DEVICES set telnetstatus="Unreachable", telnetcolour="FF0000" where ipaddress=? and topology="DNAC-TOP-1"',
                        [ip[0]])
                    con.commit()
            else:
                print("Reachability of IP address " + ip[0] + ": " + "Unreachable")
                cur.execute('update DEVICES set reachability="Unreachable", pingcolour="FF0000", telnetstatus="Unreachable", telnetcolour="FF0000" where ipaddress=? and topology="DNAC-TOP-1"', [ip[0]])
                con.commit()

            if response_2 == 0:
                print("Reachability of IP address " + ip[0] + ": " + "Reachable")
                cur.execute('update DEVICES set snmpstatus="Reachable", snmpcolour="lightblue" where ipaddress=? and topology="DNAC-TOP-1"', [ip[0]])
                con.commit()
            else:
                print("Reachability of IP address " + ip[0] + ": " + "Unreachable")
                cur.execute('update DEVICES set snmpstatus="Unreachable", snmpcolour="FF0000" where ipaddress=? and topology="DNAC-TOP-1"', [ip[0]])
                con.commit()


    def ping_dnacset2(self, con):
        cur = con.cursor()
        print("Ping reachability check on DNAC-SET-2 topology devices")
        cur.execute('select ipaddress, snmpread, username, password, enablepassword from DEVICES where topology="DNAC-TOP-2"')
        iplist = [row for row in cur]
        #print(iplist)
        for ip in iplist:
            response = os.system("ping -c 1 " + str(ip[0]))
            ...
            if response == 0:
                print("Reachability of IP address " + ip[0] + ": " + "Reachable")
                cur.execute('update DEVICES set reachability="Reachable", pingcolour="orange" where ipaddress=? and topology="DNAC-TOP-2"',
                            [ip[0]])
                con.commit()
                ...
                try:
                    ...
                    device.enable()
                    cur.execute(
                        'update DEVICES set telnetstatus="Reachable", telnetcolour="orange" where ipaddress=? and topology="DNAC-TOP-2"',
                        [ip[0]])
                    con.commit()
                    print("Device Connected")
                    device.disconnect()
                    ...
                    print(e)
                    cur.execute(
                        'update DEVICES set telnetstatus="Unreachable", telnetcolour="FF0000" where ipaddress=? and topology="DNAC-TOP-2"',
                        [ip[0]])
                    con.commit()

            else:
                print("Reachability of IP address " + ip[0] + ": " + "Unreachable")
                cur.execute('update DEVICES set reachability="Unreachable", pingcolour="FF0000", telnetstatus="Unreachable", telnetcolour="FF0000" where ipaddress=? and topology="DNAC-TOP-2"',
                            [ip[0]])
                con.commit()

            if response_2 == 0:
                print("Reachability of IP address " + ip[0] + ": " + "Reachable")
                cur.execute('update DEVICES set snmpstatus="Reachable", snmpcolour="orange" where ipaddress=? and topology="DNAC-TOP-2"',
                            [ip[0]])
                con.commit()
            else:
                print("Reachability of IP address " + ip[0] + ": " + "Unreachable")
                cur.execute('update DEVICES set snmpstatus="Unreachable", snmpcolour="FF0000" where ipaddress=? and topology="DNAC-TOP-2"',
                            [ip[0]])
                con.commit()
    ...
    def get_component_data_set1(self, con, component):
        ...
        cur.execute('select devicetype,rvsv, ipaddress, productid, reachability, snmpstatus, telnetstatus, pingcolour, snmpcolour, telnetcolour from DEVICES where component=? AND topology="DNAC-TOP-1"', [component])
        return list(cur)

    def get_component_data_set2(self, con, component):
        cur = con.cursor()
        print("Fetching DNAC Component: " + component +"data")
        cur.execute('select devicetype,rvsv, ipaddress, productid, reachability, snmpstatus, telnetstatus, pingcolour, snmpcolour, telnetcolour from DEVICES where component=? AND topology="DNAC-TOP-2"', [component])
        return list(cur)
...
if __name__ == "__main__":
   ...
   # thread_2.join()
    ...
    for key, value in dict_component.items():
        y = y + 1
        print(key, value)
        Summary_Comp = "<tr><td>" + str(y) + "</td><td>" + str(key) + "</td><td bgcolor=lightblue>" + str(value[0]) + "</td><td bgcolor=lightblue>" + str(value[1]) + "</td><td bgcolor=lightblue>" + str(value[3]) + "</td><td bgcolor=lightblue>" + str(value[2]) + "</td><td bgcolor=orange>" + str(value[4]) + "</td><td bgcolor=orange>" + str(value[5]) + "</td><td bgcolor=orange>" + str(value[7]) + "</td><td bgcolor=orange>" + str(value[6]) + "</td></tr>"
        Summary = Summary + Summary_Comp
        ...
        for value in compwise:
            ...
            for value2 in compwise2:
                if value[2] == value2[2]:
                    ...
                    x = 1
                    print(x)
                    value3 = [value2[4], value2[5], value2[6], value2[7], value2[8], value2[9]]
                    break
            if x == 1:
                y = y + 1
                WriteTable = "<tr><td>" + str(y) + "</td><td>" + str(value[0]) + "</td><td>" + str(
                    value[1]) + "</td><td>" + str(value[2]) + "</td><td bgcolor=" + str(value[7]) + ">" + str(value[4]) + "</td><td bgcolor=" + str(value[8]) + ">" + str(
                    value[5]) + "</td><td bgcolor=" + str(value[9]) + ">" + str(value[6]) + "</td><td <td bgcolor=" + str(value3[3]) + ">" + str(value3[0]) + "</td><td bgcolor=" + str(value3[4]) + ">" + str(
                    value3[1]) + "</td><td bgcolor=" + str(value3[5]) + ">" + str(value3[2]) + "</td></tr>"
                Comphtml = Comphtml + WriteTable
            else:
                y = y + 1
                WriteTable = "<tr><td>" + str(y) + "</td><td>" + str(value[0]) + "</td><td>" + str(
                    value[1]) + "</td><td>" + str(value[2]) + "</td><td bgcolor=" + str(value[7]) + ">" + str(value[4]) + "</td><td bgcolor=" + str(value[8]) + ">" + str(
                    value[5]) + "</td><td bgcolor=" + str(value[9]) + ">" + str(value[6]) + "</td><td></td><td></td><td></td></tr>"
                Comphtml = Comphtml + WriteTable
